...
/** 문의 상세 (사용자) */
export const useInquiryDetail = (inquiryNo) => {
  return useQuery({
...
export const useCreateInquiry = () => {
  ...
  return useMutation({
    mutationFn: (inquiry) => createInquiry(inquiry),
    onSuccess: () => {
...
export const useDeleteInquiry = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (inquiryNo) => deleteInquiry(inquiryNo),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['inquiry', 'my'] });
    },
  });
...
/** 관리자 문의 상세 */
export const useAdminInquiryDetail = (inquiryNo) => {
  return useQuery({
...
export const useAnswerInquiry = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ inquiryNo, answer }) => answerInquiry(inquiryNo, answer),
    onSuccess: () => {
...
export const useDeleteAdminInquiry = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (inquiryNo) => deleteAdminInquiry(inquiryNo),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin', 'inquiry'] });
    },
  });
