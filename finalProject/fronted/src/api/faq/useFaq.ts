...
/** FAQ 목록 (사용자용) */
export const useFaqList = (category: string | null = null, search = '') => {
  return useQuery({
...
export const useIncreaseFaqView = () => {
  return useMutation({
    mutationFn: (faqNo: number | string) => increaseFaqView(faqNo),
  });
};
...
export const useCreateFaq = () => {
  ...
  return useMutation({
    mutationFn: (faq: Partial<Faq>) => createFaq(faq),
    onSuccess: () => {
...
export const useUpdateFaq = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ faqNo, ...faq }: Partial<Faq> & { faqNo: number | string }) =>
      updateFaq(faqNo, faq),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin', 'faq'] });
      queryClient.invalidateQueries({ queryKey: ['faq'] });
...
export const useDeleteFaq = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (faqNo: number | string) => deleteFaq(faqNo),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin', 'faq'] });
      queryClient.invalidateQueries({ queryKey: ['faq'] });
    },
