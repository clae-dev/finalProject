...
export const useAccommodationReviews = (accommodationNo, page = 1, size = 5) => {
  return useQuery({
...
export const useReviewSummary = (accommodationNo) => {
  return useQuery({
...
export const useCreateAccommodationReview = (accommodationNo) => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (formData) => createAccommodationReview(accommodationNo, formData),
    onSuccess: () => {
...
export const useDeleteAccommodationReview = (accommodationNo) => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (reviewNo) => deleteAccommodationReview(accommodationNo, reviewNo),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['accommodationReviews', accommodationNo] });
      queryClient.invalidateQueries({ queryKey: ['reviewSummary', accommodationNo] });
    },
