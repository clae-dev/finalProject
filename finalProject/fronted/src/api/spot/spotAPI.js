...
// 활성 명소 목록 (메인페이지용)
export const getActiveSpots = async () => {
...
// 명소 상세 조회
export const getSpotDetail = async (spotNo) => {
...
export const updateSpot = async (spotNo, formData) => {
...
// 명소 삭제
export const deleteSpot = async (spotNo) => {
  const response = await axiosApi.delete(`/api/spot/${spotNo}`);
  return response.data;
};

// 명소 상태 변경
export const updateSpotStatus = async (spotNo, status) => {
