...
export const useCreateSpot = () => {
  ...
  return useMutation({
    mutationFn: (formData) => createSpot(formData),
    onSuccess: () => {
...
export const useUpdateSpot = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ spotNo, formData }) => updateSpot(spotNo, formData),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['spots'] });
    },
  });
...
export const useDeleteSpot = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (spotNo) => deleteSpot(spotNo),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['spots'] });
    },
  });
...
export const useUpdateSpotStatus = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ spotNo, status }) => updateSpotStatus(spotNo, status),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['spots'] });
    },
  });
