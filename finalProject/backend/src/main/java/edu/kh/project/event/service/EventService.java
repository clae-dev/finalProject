...
public class EventService {
    ...
    public Map<String, Object> searchFestivals(int page, int size, String eventStartDate) {
        ...
        String url = UriComponentsBuilder.fromHttpUrl(FESTIVAL_API_URL)
    ...
    /**
     * 제주 레포츠/액티비티 목록 조회 (contentTypeId=28)
     * @param page 페이지 번호
     * @param size 페이지 당 항목 수
     * @return { list, totalCount }
     */
    public Map<String, Object> searchLeisure(int page, int size) {
        ...
        log.info("===== TourAPI areaBasedList2 (레포츠) 호출 =====");
        ...
        try {
            ...
            String resultCode = String.valueOf(header.get("resultCode"));
            if (!"0000".equals(resultCode)) {
                log.warn("TourAPI 레포츠 응답 에러: {} - {}", resultCode, header.get("resultMsg"));
                result.put("list", new ArrayList<>());
                result.put("totalCount", 0);
                return result;
            }
            ...
            if (totalCount > 0 && body.get("items") != null) {
                ...
                for (Map<String, Object> item : itemList) {
                    ...
                    leisure.put("title", item.get("title"));
                    leisure.put("addr", item.get("addr1"));
                    leisure.put("image", item.get("firstimage"));
                    leisure.put("tel", item.get("tel"));
                    leisure.put("contentId", item.get("contentid"));
                    leisure.put("contentTypeId", item.get("contenttypeid"));
                    list.add(leisure);
                }
            }
            ...
            log.info("TourAPI 레포츠 조회 성공 - totalCount: {}, 현재 페이지 항목: {}", totalCount, list.size());
            return result;

        } catch (Exception e) {
            log.error("TourAPI 레포츠 호출 실패: {}", e.getMessage(), e);
            result.put("list", new ArrayList<>());
            result.put("totalCount", 0);
            return result;
        }
